<?php
	...
	abstract class FS_Migration_Abstract {
		...
		public function do_migrate_install(
			...
			$this->set_local_install( $local_install );
			...
			if ( false === $license_id ) {
				throw new Exception( 'Calling do_migrate_install() before the license was fully migrated with do_migrate_license() is not supported.' );
			}
			...
			$install = $this->sync_install( $customer_id, $license_id );

			if ( false === $install ) {
				return false;
			}

			$install_id = is_object( $install ) ?
				$install->id :
				$install;
			...
			if ( ! is_object( $install ) ) {
				$result = $this->api_call( "/installs/{$install_id}.json" );

				if ( $this->log_on_error( $result, "Failed to fetch install ({$install_id}) from Freemius." ) ) {
					return false;
				}

				$install = new FS_Install( $result );
			}
		 ...
		 * @return string
		 */
		abstract protected function get_local_install_id();
		 ...
		 * @return array
		 */
		abstract protected function get_subscription_renewal_for_api( $index = 0 );
		...
		protected function sync_subscription( $customer_id ) {
			...
			$subscription_id = $this->get_remote_id(
		 ...
		 * @param number $license_id
		 *
		 * @return false|number|\FS_Install
		 */
		protected function sync_install( $customer_id, $license_id ) {
			...
			if ( FS_Entity::is_valid_id( $install_id ) ) {
				...
				if ( $license_id != $install_current_license_id ) {
					 ...
					 */
					$this->migrate_new_license_activation( $install_id, $license_id );
				}

				return $install_id;
			}

			$install = $this->migrate_install( $customer_id, $license_id );

			if ( $this->log_on_error( $install, "Failed creating module install ({$local_install_id}) on Freemius." )
			) {
				return false;
			}

			$this->log_success( "Successfully created install ({$local_install_id}) on Freemius ({$install->id})." );
...
//			$this->link_entity( $user, $this->get_local_module_id() . ':' . $this->get_local_customer_id() );
		 ...
		 * @throws Exception
		 */
		protected function migrate_new_license_activation( $install_id, $license_id ) {
			$result = $this->api_call(
		 ...
		 * @return \FS_Payment
		 */
		protected function migrate_subscription_renewal( $subscription_id, $index ) {
			return $this->migrate_subscription_payment(
				$subscription_id,
				$this->get_subscription_renewal_for_api( $index ),
